...
public class Bidule extends JPanel {
	...
	/**
	 * utilisé pour déterminer une valeur 'aléatoire' du DELAI ayant un impact
	 * sur le déplacement
	 */
	...
	/*
	 * TODO: améliorer de toString de la couleur.
	 * 
	 */
	
	@Override
	public String toString() {
		return name + " est de couleur " + this.getBackground().toString();
	}
	...
	public void tuEstouchePar(Bidule biduleImpacteur) {
		...
		// TODO (plus difficile) : augmenter la taille de biduleImpacteur (dans
		// la
		...
		if (getWidth() < NB_MINMAL_PIXELS_VIE
				|| getHeight() < NB_MINMAL_PIXELS_VIE) {
			// sucide...
			this.stop();
	...
	/**
	 * établir une stratégie après impact ; un autre bidule vient de (me)
	 * toucher (toucher l'objet courant)
	 */
	...
	/**
	 * Obtenir les objets de la scence à une certaine distance de l'objet
	 * courant
	 * 
	 * @param distance
	 *            en pixel
	...
	protected List<Bidule> getBidulesProches(int distance) {
		...
		mySpace.setBounds(mySpace.x - distance, mySpace.y - distance,
				mySpace.width + distance, mySpace.height + distance);
